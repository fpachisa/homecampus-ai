...
const MathText: React.FC<MathTextProps> = ({ children, className = '' }) => {
  ...
  //   - textWithPlaceholders: Text after LaTeX extraction (shows ⟨⟨MATH0⟩⟩ placeholders)
  ...
  //   - Shows if LaTeX placeholders are inside <li> tags: "<li>Length = ⟨⟨MATH0⟩⟩</li>"
  ...
  /*
  if (hasBlockMarkdown) {
    console.log('🔍 MathText rendered HTML:', {
 ...
 * - Examples that MATCH: $x$ ❌ (no backslash), $\theta$ ✓, $38^{\circ}$ ✓
 *
 * WAIT - THIS IS A BUG!
...
function processInlineMathAndFractions(text: string): TextSegment[] {
  ...
  // - Much simpler and catches all LaTeX: $x$, $\theta$, $38^{\circ}$, $\frac{1}{2}$ ✓
  //
  ...
  // EXAMPLES:
  // - $x$ ✓ (simple variable)
  // - $38^{\circ}$ ✓ (superscript with symbol)
  // - $\frac{1}{2}$ ✓ (fraction)
  // - $3.50 ✓ (will be rendered as math, which is probably wrong for money)
  // - $100 ✓ (will be rendered as math)
  //
  // WHY THIS IS OK:
